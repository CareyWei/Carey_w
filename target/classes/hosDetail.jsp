    ...
    <div class="wrap">
        ...
        <div class="container" data-accuracy="0">
            <article class="hospi-top">
                <div class="hospi-pic">
                    <img src="/images/index/yisheng1.png">
                </div>
                <div class="hospi-info">
                    <div class="clearfix">
                        <h3 class="hospi-title fl"><span id="hospital_name">${HosGet.hName}</span><span
                                class="grade">${HosGet.hLevel}</span></h3>
                        <p class="grade-star fr"><i class="star star-light star-fixed"></i><i
                                ...
                                class=""></em><span class="prompt-down">医院综合评分：由网络用户投票得出<i></i></span></p>
                    </div>
                    <div class="hospi-content">
                        <p class="text"><span class="label">医院电话：</span><label
                                id="hospital_phone">${HosGet.hPhone}</label></p>
                        <p class="text"><span class="label">医院地址：</span><label
                                id="hospital_address">${HosGet.hAddress}</label></p>
                        <div class="hos-short text"><span class="label">医院概况：</span>
                            <p class="detail key">${HosGet.hInformation}</p>
                        </div>
                        <p class="text s-info"><span class="label">提供服务：</span><span class="key">预约挂号</span>&nbsp;/&nbsp;<span
                                class="key">出诊提醒</span></p>
                        <p class="ilt"><span class="num"></span>位优质医生<b class="splitter"></b><span class="num"></span>人成功预
                        </p>
                    </div>
                </div>
            </article>
                ...
                <div class="main">
                    ...
                    <div class="content  current otiotns" style="display: block;">
                        <div class="select-condition hospital-detail">
                            <div class="select-detail" data-type="1">
                                ...
                                <ul class="dropmenus" id="depart-level1-ul" style="margin-bottom: 10px;">
                                    <li data-department-flag="1" data-value="0" id="depart0" <c:if
                                            test="${empty did }"> class="select-cur" </c:if> data-child="1">
                                        <a href="/hospital/detail/${hId}?grade=${grade}">全部</a></li>
                                    <c:forEach items="${dept}" var="dt">
                                        <li data-department-flag="1"  <c:if test="${did == dt.did }"> class="select-cur" </c:if> id="${dt.dname}">
                                            <a href="/hospital/detail/${hId}?key=${key}&grade=${grade}&did=${dt.did}">${dt.dname}</a>
                                        </li>
                                    </c:forEach>
                                ...
                                <ul class="dropmenus" id="depart-level2-ul" style="margin-bottom: 10px;">
                                    <li data-department-flag="1" data-value="0" id="depart0" <c:if
                                            test="${empty dTid }"> class="select-cur" </c:if> data-child="1"><a
                                            href="/hospital/detail/${hId}?grade=${grade}">全部</a></li>
                                    <c:forEach items="${deptT}" var="dT">
                                        <li data-department-flag="1"  <c:if
                                                test="${dTid == dT.dTid }"> class="select-cur" </c:if>
                                            id="${dT.dTname}">
                                            <a href="/hospital/detail/${hId}?key=${key}&grade=${grade}&dTid=${dT.dTid}&did=">${dT.dTname}</a>
                                        </li>
                                    </c:forEach>
                                </ul>
                            ...
                            <div class="select-detail mains-doctor" data-type="3"
                                 style="border-top-width: 1px; border-top-style: dashed; border-top-color: rgb(234, 234, 234);">
                                <h4>医生职称:</h4>
                                <ul class="dropmenus" id="medtitle-ul">
                                    <li data-value="0" id="title0" <c:if
                                            test="${empty grade}"> class="select-cur" </c:if> ><a
                                            href="/hospital/detail/${hId}?key=${key}&did=${did}&dTid=${dTid}">全部</a></li>
                                    <li data-value="1" id="title1"
                                        <c:if test="${ grade==1}">class="select-cur"</c:if>  ><a
                                            href="/hospital/detail/${hId}?key=${key}&grade=1&did=${did}&dTid=${dTid}">主任医师</a></li>
                                    <li data-value="1" id="title2"
                                        <c:if test="${ grade==2}">class="select-cur"</c:if> ><a
                                            href="/hospital/detail/${hId}?key=${key}&grade=2&did=${did}&dTid=${dTid}">副主任医师</a></li>
                                    <li data-value="3" id="title3"
                                        <c:if test="${ grade==3}">class="select-cur"</c:if> ><a
                                            href="/hospital/detail/${hId}?key=${key}&grade=3&did=${did}&dTid=${dTid}">主治医师</a></li>
                                    <li data-value="4" id="title4"
                                        <c:if test="${ grade==4}">class="select-cur"</c:if> ><a
                                            href="/hospital/detail/${hId}?key=${key}&grade=4&did=${did}&dTid=${dTid}">普通医师</a></li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="doctor-info-box">
                            ...
                            <ul id="doctor-info-list" class="doctor-info-list" style="display: block;">
                                ...
                                <c:forEach items="${page.data}" var="doctor">
                                    <li class="doctor-info-single"
                                        style="border-top-width: 1px; border-right-width: 1px; border-left-width: 1px; border-style: solid solid none; border-top-color: rgb(234, 234, 234); border-right-color: rgb(255, 255, 255); border-left-color: rgb(255, 255, 255);">
                                        <div class="doctor-card clearfix">
                                            <div class="info-pic"><a href="doctor/detail/${doctor.hId}" target="_blank">
                                                <img src="/images/index/yisheng1.png"></a></div>
                                            <div class="infos">
                                                <div class="info-title" style="margin-top: -10px"><a href="#"
                                                                                                     target="_blank">
                                                    <div>${doctor.name}</div>
                                             ...
                                             <c:choose>
                                                 ...
                                                 <c:when test="${doctor.grade == 4}">普通医师</c:when>
                                        ...
                                        </span>/<span class="doctor-title">教授，博士生导师</span>
                                                    <p class="info-hospittal"><span><a
                                                            href="hospital/detail/${doctor.hospital.hId}"
                                                            target="_blank">${doctor.hospital.hName}</a></span><a
                                                            href="#" target="_blank">${doctor.deptlist.dname}/${doctor.deptlistT.dTname}</a></p>
                                                    <table cellspacing="0" cellpadding="0">
                                                        <tbody>
                                                        <tr>
                                                            <td class="first-td" valign="top">医院地址:</td>
                                                            <td class="last-td"
                                                                valign="top">${doctor.hospital.hAddress}</td>
                                                        </tr>
                                                        <tr>
                                                    ...
                                                    <div class="rate"><span>总推荐度：</span>
                                                        <div class="grade-star"><i
                                                                class="star-right star-light star-fixed"></i><i
                                                                class="star-right star-light star-fixed"></i><i
                                                                class="star-right star-light star-fixed"></i><i
                                                                class="star-right star-light star-fixed"></i><i
                                                                class="star-right star-gray-light star-fixed"></i></div>
                                                    </div>
                                                </div>
                                            </div>
                ...
                <span class="pagers">
                    <ul class="pagination-x-list">
                        <ul>
                                <li>
                                   <a class="dropmenus"
                                      href="hospital/detail/${hId}?grade=${grade}&did=${did}&dTid=${dTid}&pageNum=1">
                                    <label>首页</label>
                                   </a>
                               </li>
                            ...
                            <c:if test="${page.pageNum != 1}">
                                <li>
                                 <a class="prev-on"
                                    href="hospital/detail/${hId}?grade=${grade}&did=${did}&dTid=${dTid}&pageNum=${page.pageNum-1}">
                                    <label>上一页</label>
                                </a>
                                </li>
                            </c:if>
                           ...
                           <a class="next-on"
                              href="hospital/detail/${hId}?grade=${grade}&did=${did}&dTid=${dTid}&pageNum=${page.pageNum+1}">
                                ...
                                <li>
                                   <a class="dropmenus"
                                      href="hospital/detail/${hId}?grade=${grade}&did=${did}&dTid=${dTid}&pageNum=${page.totalPage}">
                                    <label>末页</label>
                                   </a>
                               </li>
                        </ul>
