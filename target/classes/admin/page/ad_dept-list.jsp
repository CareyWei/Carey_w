...
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="/DeptMessage">
    ...
    <xblock>
        ...
        <a class="layui-btn" href="/selectInsertDeptT"><i class="layui-icon"></i>添加
        </a>
        ...
        <tr>
            <td>
                <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='2'>
                    <input type="checkbox" id="checkbox_item" class="id" name="id" value="${dept.dTid}"></div>
            </td>
            <td>${dept.dTid}</td>
            <td>${dept.dTname}</td>
            <td>${dept.deptlist.did}</td>
            <td>${dept.deptlist.dname}</td>
            <td class="td-manage">
                <a title="删除" href="javascript:;" onclick="return deleteKey(${dept.dTid});">
                    <i class="layui-icon" >删除</i>
                </a>||
                <a title="编辑" href="/selectUpdateDept?dTid=${dept.dTid}&dTname=${dept.dTname}" >
      ...
      <span class="pagers">
            ...
            <ul class="pagination-x-list">
                <ul>
                    <li>
                        <a class="dropmenus"
                           href="/DeptMessage?pageNum=${Dpage.firstPage}&key=${key}&grade=${grade}&did=${did}&dTid${dTid}">
                            <label>首页</label>
                        </a>
                    </li>
                    ...
                    <c:if test="${Dpage.pageNum != 1}">
                        <li>
                            <a class="prev-on"
                               href="/DeptMessage?pageNum=${Dpage.pageNum-1}&key=${key}&grade=${grade}&did=${did}&dTid${dTid}">
                                <label>上一页</label>
                            </a>
                        </li>
                    </c:if>
                    ...
                    <li>
                        <a class="dropmenus"
                           href="/DeptMessage?pageNum=${Dpage.lastPage}&key=${key}&grade=${grade}&did=${did}&dTid=${dTid}">
                            <label>末页</label>
                        </a>
                    </li>
                </ul>
...
<script>
    ...
    $("#delete_all_btn").click(function () {
        ...
        if (c ==true) {
            $.ajax({//利用ajax发出请求
                type: "get",//请求类型
                url: "/deptT/delete", //向Controller里的batchDeleYyb传输ids
                        data: {dTid: a},     //键值对
