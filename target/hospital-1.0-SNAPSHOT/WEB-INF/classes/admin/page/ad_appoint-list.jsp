...
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so"  method="get" action="/AppointMessage">
            ...
            <tr>
                <td>
                    <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='2'>
                        <input type="checkbox" id="checkbox_item" class="id" name="id" value="${appointment.apId}"></div>
                </td>
                <td>${appointment.apId}</td>
                <td>${appointment.uid}</td>
                <td>${appointment.apName}</td>
                <td>${appointment.idcast}</td>
                <td>${appointment.apAddress}</td>
                ...
                <td class="td-manage">
                    <a title="删除" href="javascript:;" onclick="return deleteKey(${appointment.apId});">
                        <i class="layui-icon" >删除</i>
                    </a>||
                    <a title="编辑" href="/selectUpdateAppoint?apId=${appointment.apId}" >
      ...
      <span class="pagers">
            ...
            <ul class="pagination-x-list">
                <ul>
                    <li>
                        <a class="dropmenus"
                           href="/AppointMessage?pageNum=${Dpage.firstPage}&key=${key}&rid=${rid}">
                            <label>首页</label>
                        </a>
                    </li>
                    ...
                    <c:if test="${Dpage.pageNum != 1}">
                        <li>
                            <a class="prev-on"
                               href="/AppointMessage?pageNum=${Dpage.pageNum-1}&key=${key}&rid=${rid}">
                                <label>上一页</label>
                            </a>
                        </li>
                    </c:if>
                    ...
                    <li>
                        <a class="dropmenus"
                           href="/AppointMessage?pageNum=${Dpage.lastPage}&key=${key}&rid=${rid}">
                            <label>末页</label>
                        </a>
                    </li>
                </ul>
...
<script>
    ...
    $("#delete_all_btn").click(function () {
        ...
        if (c ==true) {
            $.ajax({//利用ajax发出请求
                type: "get",//请求类型
                url: "/appoint/delete", //向Controller里的batchDeleYyb传输ids
                        data: {apId: a},     //键值对
