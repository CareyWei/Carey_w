...
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="/UserMessage">
            ...
            <tr>
                <td>
                    <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='2'>
                        <input type="checkbox" id="checkbox_item" class="id" name="id" value="${user.uid}"></div>
                </td>
                <td>${user.uid}</td>
                <td>${user.name}</td>
                <td>${user.email}</td>
                ...
                <td class="td-manage">
                    <a title="删除" href="javascript:;" onclick="return deleteKey(${user.uid});">
                        <i class="layui-icon" >删除</i>
                    </a>||
                    <a title="编辑" href="/selectUpdateUser?uid=${user.uid}" >
      ...
      <span class="pagers">
            ...
            <ul class="pagination-x-list">
                <ul>
                    <li>
                        <a class="dropmenus"
                           href="/UserMassage?pageNum=${Dpage.firstPage}&key=${key}&grade=${grade}&did=${did}&dTid${dTid}">
                            <label>首页</label>
                        </a>
                    </li>
                    ...
                    <c:if test="${Dpage.pageNum != 1}">
                        <li>
                            <a class="prev-on"
                               href="/UserMassage?pageNum=${Dpage.pageNum-1}&key=${key}&grade=${grade}&did=${did}&dTid${dTid}">
                                <label>上一页</label>
                            </a>
                        </li>
                    </c:if>
                    ...
                    <li>
                        <a class="dropmenus"
                           href="/UserMassage?pageNum=${Dpage.lastPage}&key=${key}&grade=${grade}&did=${did}&dTid=${dTid}">
                            <label>末页</label>
                        </a>
                    </li>
                </ul>
...
<script>
    ...
    $("#delete_all_btn").click(function () {
        ...
        if (c ==true) {
            $.ajax({//利用ajax发出请求
                type: "get",//请求类型
                url: "/tuser/delete", //向Controller里的batchDeleYyb传输ids
                        data: {uid: a},     //键值对
